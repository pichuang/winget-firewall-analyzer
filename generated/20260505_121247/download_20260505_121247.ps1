...
$PkgDir = "$DownloadDir\Microsoft\AccessDatabaseEngine2016"
New-Item -ItemType Directory -Path $PkgDir -Force | Out-Null
Write-Host "📦 Microsoft.AccessDatabaseEngine2016 v16.0.5044.1000" -ForegroundColor White

$FilePath = "$PkgDir\accessdatabaseengine.exe"
if (Test-Path $FilePath) {
    Write-Host "   ⏭️  已存在: accessdatabaseengine.exe (x86)" -ForegroundColor DarkGray
    $Skipped++
} else {
    Write-Host "   ⬇️  下載中: accessdatabaseengine.exe (x86/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/3/5/c/35c84c36-661a-44e6-9324-8786b8dbe231/accessdatabaseengine.exe" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
        Write-Host "   ❌ 下載失敗: accessdatabaseengine.exe" -ForegroundColor Red
        $Failed++
        Remove-Item $FilePath -ErrorAction SilentlyContinue
    }
}

$FilePath = "$PkgDir\accessdatabaseengine_X64.exe"
if (Test-Path $FilePath) {
    Write-Host "   ⏭️  已存在: accessdatabaseengine_X64.exe (x64)" -ForegroundColor DarkGray
    $Skipped++
} else {
    Write-Host "   ⬇️  下載中: accessdatabaseengine_X64.exe (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/3/5/c/35c84c36-661a-44e6-9324-8786b8dbe231/accessdatabaseengine_X64.exe" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
        Write-Host "   ❌ 下載失敗: accessdatabaseengine_X64.exe" -ForegroundColor Red
        $Failed++
        Remove-Item $FilePath -ErrorAction SilentlyContinue
    }
}
...
$PkgDir = "$DownloadDir\Microsoft\AdvertisingEditor"
New-Item -ItemType Directory -Path $PkgDir -Force | Out-Null
Write-Host "📦 Microsoft.AdvertisingEditor v11.31.21766" -ForegroundColor White

$FilePath = "$PkgDir\MicrosoftAdvertisingEditor.exe"
if (Test-Path $FilePath) {
    Write-Host "   ⏭️  已存在: MicrosoftAdvertisingEditor.exe (x64)" -ForegroundColor DarkGray
    $Skipped++
} else {
    Write-Host "   ⬇️  下載中: MicrosoftAdvertisingEditor.exe (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://prod.editor.ads.microsoft.com/download/production-pc/c/MicrosoftAdvertisingEditor.exe" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
        Write-Host "   ❌ 下載失敗: MicrosoftAdvertisingEditor.exe" -ForegroundColor Red
        $Failed++
        Remove-Item $FilePath -ErrorAction SilentlyContinue
    }
}
...
New-Item -ItemType Directory -Path $PkgDir -Force | Out-Null
Write-Host "📦 Microsoft.AzureCLI v2.86.0" -ForegroundColor White

$FilePath = "$PkgDir\azure-cli-2.86.0.msi"
if (Test-Path $FilePath) {
    Write-Host "   ⏭️  已存在: azure-cli-2.86.0.msi (x86)" -ForegroundColor DarkGray
    $Skipped++
} else {
    Write-Host "   ⬇️  下載中: azure-cli-2.86.0.msi (x86/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://azcliprod.blob.core.windows.net/msi/azure-cli-2.86.0.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
        Write-Host "   ❌ 下載失敗: azure-cli-2.86.0.msi" -ForegroundColor Red
        $Failed++
        Remove-Item $FilePath -ErrorAction SilentlyContinue
    }
}

$FilePath = "$PkgDir\azure-cli-2.86.0-x64.msi"
if (Test-Path $FilePath) {
    Write-Host "   ⏭️  已存在: azure-cli-2.86.0-x64.msi (x64)" -ForegroundColor DarkGray
    $Skipped++
} else {
    Write-Host "   ⬇️  下載中: azure-cli-2.86.0-x64.msi (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://azcliprod.blob.core.windows.net/msi/azure-cli-2.86.0-x64.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
        Write-Host "   ❌ 下載失敗: azure-cli-2.86.0-x64.msi" -ForegroundColor Red
        $Failed++
        Remove-Item $FilePath -ErrorAction SilentlyContinue
    }
}
...
New-Item -ItemType Directory -Path $PkgDir -Force | Out-Null
Write-Host "📦 Microsoft.Bicep v0.43.1" -ForegroundColor White
...
if (Test-Path $FilePath) {
    ...
    try {
        Invoke-WebRequest -Uri "https://github.com/Azure/bicep/releases/download/v0.43.1/bicep-win-x64.exe" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    try {
        Invoke-WebRequest -Uri "https://github.com/Azure/bicep/releases/download/v0.43.1/bicep-win-arm64.exe" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    try {
        Invoke-WebRequest -Uri "https://github.com/Azure/bicep/releases/download/v0.43.1/bicep-setup-win-x64.exe" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
$PkgDir = "$DownloadDir\Microsoft\IIS\ApplicationRequestRouting"
New-Item -ItemType Directory -Path $PkgDir -Force | Out-Null
Write-Host "📦 Microsoft.IIS.ApplicationRequestRouting v3.0.05311" -ForegroundColor White

$FilePath = "$PkgDir\fwlink"
if (Test-Path $FilePath) {
    Write-Host "   ⏭️  已存在: fwlink (x86)" -ForegroundColor DarkGray
    $Skipped++
} else {
    Write-Host "   ⬇️  下載中: fwlink (x86/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://go.microsoft.com/fwlink/?LinkID=615135" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
$FilePath = "$PkgDir\fwlink"
if (Test-Path $FilePath) {
    Write-Host "   ⏭️  已存在: fwlink (x64)" -ForegroundColor DarkGray
    $Skipped++
} else {
    Write-Host "   ⬇️  下載中: fwlink (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://go.microsoft.com/fwlink/?LinkID=615136" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
New-Item -ItemType Directory -Path $PkgDir -Force | Out-Null
Write-Host "📦 Microsoft.OpenJDK.17 v17.0.19.10" -ForegroundColor White

$FilePath = "$PkgDir\microsoft-JDK-17.0.19-windows-x64.msi"
if (Test-Path $FilePath) {
    Write-Host "   ⏭️  已存在: microsoft-JDK-17.0.19-windows-x64.msi (x64)" -ForegroundColor DarkGray
    $Skipped++
} else {
    Write-Host "   ⬇️  下載中: microsoft-JDK-17.0.19-windows-x64.msi (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://aka.ms/download-JDK/microsoft-JDK-17.0.19-windows-x64.msi#winget" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
        Write-Host "   ❌ 下載失敗: microsoft-JDK-17.0.19-windows-x64.msi" -ForegroundColor Red
        $Failed++
        Remove-Item $FilePath -ErrorAction SilentlyContinue
    }
}

$FilePath = "$PkgDir\microsoft-JDK-17.0.19-windows-aarch64.msi"
if (Test-Path $FilePath) {
    Write-Host "   ⏭️  已存在: microsoft-JDK-17.0.19-windows-aarch64.msi (arm64)" -ForegroundColor DarkGray
    $Skipped++
} else {
    Write-Host "   ⬇️  下載中: microsoft-JDK-17.0.19-windows-aarch64.msi (arm64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://aka.ms/download-JDK/microsoft-JDK-17.0.19-windows-aarch64.msi#winget" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
        Write-Host "   ❌ 下載失敗: microsoft-JDK-17.0.19-windows-aarch64.msi" -ForegroundColor Red
        $Failed++
        Remove-Item $FilePath -ErrorAction SilentlyContinue
    }
}
...
New-Item -ItemType Directory -Path $PkgDir -Force | Out-Null
Write-Host "📦 Microsoft.OpenJDK.21 v21.0.11.10" -ForegroundColor White

$FilePath = "$PkgDir\microsoft-JDK-21.0.11-windows-x64.msi"
if (Test-Path $FilePath) {
    Write-Host "   ⏭️  已存在: microsoft-JDK-21.0.11-windows-x64.msi (x64)" -ForegroundColor DarkGray
    $Skipped++
} else {
    Write-Host "   ⬇️  下載中: microsoft-JDK-21.0.11-windows-x64.msi (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://aka.ms/download-JDK/microsoft-JDK-21.0.11-windows-x64.msi#winget" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
        Write-Host "   ❌ 下載失敗: microsoft-JDK-21.0.11-windows-x64.msi" -ForegroundColor Red
        $Failed++
        Remove-Item $FilePath -ErrorAction SilentlyContinue
    }
}

$FilePath = "$PkgDir\microsoft-JDK-21.0.11-windows-aarch64.msi"
if (Test-Path $FilePath) {
    Write-Host "   ⏭️  已存在: microsoft-JDK-21.0.11-windows-aarch64.msi (arm64)" -ForegroundColor DarkGray
    $Skipped++
} else {
    Write-Host "   ⬇️  下載中: microsoft-JDK-21.0.11-windows-aarch64.msi (arm64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://aka.ms/download-JDK/microsoft-JDK-21.0.11-windows-aarch64.msi#winget" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
        Write-Host "   ❌ 下載失敗: microsoft-JDK-21.0.11-windows-aarch64.msi" -ForegroundColor Red
        $Failed++
        Remove-Item $FilePath -ErrorAction SilentlyContinue
    }
}
...
$PkgDir = "$DownloadDir\Microsoft\eBPFforWindows"
New-Item -ItemType Directory -Path $PkgDir -Force | Out-Null
Write-Host "📦 Microsoft.eBPFforWindows v1.1.0" -ForegroundColor White

$FilePath = "$PkgDir\Build-native-only.NativeOnlyRelease.x64.zip"
if (Test-Path $FilePath) {
    Write-Host "   ⏭️  已存在: Build-native-only.NativeOnlyRelease.x64.zip (x64)" -ForegroundColor DarkGray
    $Skipped++
} else {
    Write-Host "   ⬇️  下載中: Build-native-only.NativeOnlyRelease.x64.zip (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://github.com/microsoft/ebpf-for-windows/releases/download/Release-v1.1.0/Build-native-only.NativeOnlyRelease.x64.zip" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
        Write-Host "   ❌ 下載失敗: Build-native-only.NativeOnlyRelease.x64.zip" -ForegroundColor Red
        $Failed++
        Remove-Item $FilePath -ErrorAction SilentlyContinue
    }
}

$FilePath = "$PkgDir\Build-native-only.NativeOnlyRelease.arm64.zip"
if (Test-Path $FilePath) {
    Write-Host "   ⏭️  已存在: Build-native-only.NativeOnlyRelease.arm64.zip (arm64)" -ForegroundColor DarkGray
    $Skipped++
} else {
    Write-Host "   ⬇️  下載中: Build-native-only.NativeOnlyRelease.arm64.zip (arm64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://github.com/microsoft/ebpf-for-windows/releases/download/Release-v1.1.0/Build-native-only.NativeOnlyRelease.arm64.zip" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
        Write-Host "   ❌ 下載失敗: Build-native-only.NativeOnlyRelease.arm64.zip" -ForegroundColor Red
        $Failed++
        Remove-Item $FilePath -ErrorAction SilentlyContinue
    }
}
...
New-Item -ItemType Directory -Path $PkgDir -Force | Out-Null
Write-Host "📦 Microsoft.msodbcsql.17 v17.11.1.1" -ForegroundColor White
...
if (Test-Path $FilePath) {
    ...
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/57ea94d1-a9bd-4ab7-990a-1c1c4e7a2ef8/x86/1033/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/d99fcd4f-548f-46e6-83d5-b9eb62b373d5/amd64/1033/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x86/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/62cf1733-644f-4e39-bfbb-f2a22e3a04b1/x86/1031/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/732b0a1e-304c-4eba-81fe-9e45ae38f3e3/amd64/1031/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x86/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/6e0b3d97-fe12-447c-b5da-a32e8ef81536/x86/3082/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/4f3f0ba4-b33d-4cdc-a833-cf443b2ef1a3/amd64/3082/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x86/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/26ecf933-f8e3-45ab-a11a-98e95677a5ca/x86/1036/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/ec01b851-6b13-4b08-812a-bd321405fe69/amd64/1036/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x86/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/7f550933-ae5d-4cbf-ac74-66677281af35/x86/1040/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/fd8ee424-7bae-463e-8f3a-435d49b33aa3/amd64/1040/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x86/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/389bad56-6590-477a-a6ba-99b46cc7e634/x86/1041/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/87e8b0bc-d558-4247-b115-f58d8f97fdc3/amd64/1041/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x86/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/0aaffa48-eec6-49fa-b964-5a3360e420d3/x86/1042/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/103a391b-2d9f-4212-ae41-9011be7a80e1/amd64/1042/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x86/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/f7e6f45a-eccf-4443-8fb6-96c37dc61bfb/x86/1046/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/44748bc5-fa9e-450f-8463-c761f9ddf6dd/amd64/1046/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x86/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/e31b4cea-bd03-4e06-aa1a-701417881b47/x86/1049/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/e1e07d39-6707-4c68-b1da-e3abf9704c5f/amd64/1049/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x86/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/d6a3132a-611f-45dc-905d-e80f55e9920c/x86/2052/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/3f00a8d7-2c56-4e74-bf1d-0a2ccca531d8/amd64/2052/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x86/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/501e7567-782d-47b0-8330-004610f0f12a/x86/1028/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
...
if (Test-Path $FilePath) {
    ...
    Write-Host "   ⬇️  下載中: msodbcsql.msi (x64/default)" -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri "https://download.microsoft.com/download/a41bcbe4-920e-40a9-bcb2-a45d2d5be1f6/amd64/1028/msodbcsql.msi" -OutFile $FilePath -UseBasicParsing
        $Size = (Get-Item $FilePath).Length / 1MB
        Write-Host ("   ✅ 完成: {0:N1} MB" -f $Size) -ForegroundColor Green
        $Total++
    } catch {
